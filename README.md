...
- 已加入火焰纹章式占位 UI：右侧单位卡、立绘框、程序绘制单位造型；正式图片资源尚未导入。
- 当前主场景：`scenes/battle/BattleScene.tscn`。
- 当前玩法：选择单位、移动、攻击、等待、结束回合、敌方占位行动、命中/暴击/克制结算、阵线减伤、地形修正、右侧单位信息栏。
...
- `assets/sprites/README.md`：角色/map sprite 占位说明，当前尚未导入正式图片。
- `assets/ui/README.md`：UI 框体与立绘资源占位说明。
